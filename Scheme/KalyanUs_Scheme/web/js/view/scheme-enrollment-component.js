    ...
    function ($, Component, alert, ko, url, priceUtils, messageList) {
        ...
        return Component.extend({
            defaults: {
                ...
                isVerifyOtp: ko.observable(false),
                ...
                isOtpSend: ko.observable(false),
                sendOtpType: ko.observable(null),
            ...
            sendOtp: function (type) {
                var self = this;
                self.sendOtpType(type);
                var dataSchemeForm = $('#scheme-enrollment-form');
                dataSchemeForm.validation();

                var dataSchemeMobileForm = $('#sch-mobile-number');
                var dataSchemeEmailForm = $('#sch-email');
                dataSchemeMobileForm.validation();
                dataSchemeEmailForm.validation();

                var mobileNo = "";
                var email = "";
                if (type === "mobile") {
                    mobileNo = $('#sch-mobile-number').val();
                } else if (type === "email") {
                    email = $('#sch-email').val();
                }
                ...
                var isValid = false;
                if (type === "mobile") {
                    isValid = dataSchemeMobileForm.validation('isValid');
                } else if (type === "email") {
                    isValid = dataSchemeEmailForm.validation('isValid');
                }
            ...
            verifyOtp: function () {
                var self = this;
                let otpType = self.sendOtpType() ? self.sendOtpType() : 'email';
                var dataMobileField = $('#sch-mobile-number');
                var dataEmailField = $('#sch-email');
                dataMobileField.validation();
                dataEmailField.validation();
                var mobileNo = "";
                var email = "";
                if (otpType === "mobile") {
                    mobileNo = $('#sch-mobile-number').val();
                ...
                let otp = $('#sch-mobile-number-otp').val();
                var status = false;
                if (otpType === "mobile") {
                    status = dataMobileField.validation('isValid');
                } else if (otpType === "email") {
                    status = dataEmailField.validation('isValid');
                }
                if (status) {
                    jQuery.ajax({
                        ...
                        type: "POST",
                        showLoader: true,
                        data: { mobile: mobileNo, email: email, otp: otp },
                        cache: false,
                        success: function (response) {
                            if (response.status == true) {
            ...
            submitform: function () {
                ...
                if (status) {
                    if (self.isVerifyOtp()) {
                        ...
                        messageList.addErrorMessage({ message: 'Please verify email.' });
                    }
                }
            },
            ...
            allowToEditEmail: function () {
                var self = this;
                if (self.isOtpSend() == true || self.isVerifyOtp() == true) {
                    return true;
                } else {
                    return false;
                }
